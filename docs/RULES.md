...
Organizational Gravity is the accumulated force of systems, processes, approvals, habits, handoffs, fragmented data, technical decisions, and knowledge dependencies that pulls people toward existing or undesirable ways of working.

For meaningful changes, determine whether the design makes the desired behavior easier, clearer, safer, and more automatic. Question friction likely to cause workarounds, duplicate work, or a return to old behavior.

Friction is justified when it provides a real control, including security, safety, financial control, compliance, contractual obligations, data integrity, architectural boundaries, or legitimate human judgment. Preserve necessary controls; remove accidental friction.

Treat these as design smells, not automatic failures:

- a new required field, approval, status, table, screen, manual handoff, or source of truth
- duplicate data, manual synchronization, or people remembering future actions
- users entering known information, consulting instructions during routine work, or navigating elsewhere to determine the next action
- business logic that exists only in training, or permissions used instead of workflow design
- dashboards without actions or notifications compensating for missing workflow
- fields kept for hypothetical use or states that drive no behavior
- exceptions forced through the normal path, or replacement processes that do not retire the old path
- automation that conceals or reproduces a flawed process faster
...
5. the gravity, behavior-change, source-of-truth, maintenance, and mode implications
...
## Governance must not create gravity
...
The Additive Change Gate is the primary gravity review and occurs before implementation. Before completion, inspect the actual diff only for unexpected user actions, fields, approvals, statuses, tables, dependencies, integrations, synchronization, training, permissions, behavior changes, sources of truth, or hidden unnecessary manual work. Fix unexpected gravity before declaring completion. Report changed files, verification, and material risks or rollback information — without creating a separate governance artifact to prove it happened.
